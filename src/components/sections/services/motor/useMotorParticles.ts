...
// Owns the entire imperative three.js lifecycle for the particle motor:
// scene/camera/renderer setup, the GLB load + cube/motor position bake,
// pointer-driven disperse via raycasting, and the render loop. `container`
// sizes the renderer and receives pointer events; `canvasWrap` is where the
// renderer's canvas element gets mounted; `formProgressRef` (0 = loose cube,
// 1 = fully formed motor) drives the cube→motor morph each frame.
export function useMotorParticles(
  containerRef: RefObject<HTMLDivElement | null>,
  canvasWrapRef: RefObject<HTMLDivElement | null>,
  formProgressRef?: RefObject<number>,
) {
  useEffect(() => {
  ...
  }, [containerRef, canvasWrapRef, formProgressRef]);
}
